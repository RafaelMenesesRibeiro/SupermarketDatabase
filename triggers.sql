...
CREATE OR REPLACE FUNCTION do_produto() RETURNS trigger as $$
	begin
		if new.forn_primario in ( select nif from fornece_sec where ean = new.ean )
		then RAISE EXCEPTION 'The primary supplier given is already a secondary supplier for this product. This cannot happen.';
		end if;
		return NEW;
	end;
$$ LANGUAGE 'plpgsql';
...
CREATE OR REPLACE FUNCTION do_reposicao() RETURNS trigger as $$
	begin
		...
		then RAISE EXCEPTION 'The restocking instant given is in the future. This cannot happen.';
		end if;
		return NEW;
	end;
$$ LANGUAGE 'plpgsql';
